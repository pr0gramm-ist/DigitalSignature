...
namespace DigitalSignature
{
    ...
    {
        private static byte[] initialVector = new byte[64];

        private static byte[] N = new byte[64];

        private static byte[] Sigma = new byte[64];

        private static ulong[] matrixA = null;
        private static byte[] sbox = null;
        private static int[] descriptionTeta = null;

        static int[] DescriptionTeta()
        {
            if (descriptionTeta is null)
            {
                return new int[]{ 0, 8, 16, 24, 32, 40, 48, 56, 1,9, 17, 25, 33, 41,49, 57,
                                2, 10, 18, 26, 34, 42, 50, 58, 3, 11, 19, 27, 35, 43, 51,
                                59, 4, 12, 20, 28, 36, 44, 52, 60, 5, 13, 21,29, 37, 45,
                                53, 61,6, 14, 22, 30, 38, 46, 54, 62, 7, 15, 23, 31,39,
                                47, 55, 63};
            }
            else
            {
                return descriptionTeta;
            }
        }
        static byte[] Sbox()
        {
        ...
        static byte[] XChanger(byte[] key, byte[] vector)
        {
            var result = new byte[64];

            for (int i = 0; i < 64; i++)
            {
                result[i] = (byte)(key[i] ^ vector[i]);
            }

            return result;
        }
        static byte[] SChanger(byte[] vector)
        {
            byte[] result = new byte[64];
            var mysbox = Sbox();
            for (int i = 0; i < 64; i++)
                result[i] = mysbox[vector[i]];
            return result;
        }
        static byte[] PChanger(byte[] vector)
        {
            byte[] result = new byte[64];
            var myDescriptionTeta = DescriptionTeta();
            for (int i = 0; i < 64; i++)
            {
                result[i] = vector[myDescriptionTeta[i]];
            }
            return result;

        }
        static byte[] LChanger(byte[] vector)
        {
            var result = new byte[64];
        ...
        static byte[] AddModulo512(byte[] a, byte[] b)
        {
        ...
        static byte[] KeySchedule(byte[] K, int i)
        {
            K = XChanger(K, CConsts()[i]);
            K = SChanger(K);
            K = PChanger(K);
            K = LChanger(K);
            return K;
        }
        static byte[] Encryption(byte[] key, byte[] m)
        {
            byte[] state = XChanger(key, m);
            for (int i = 0; i < 12; i++)
            {
                state = SChanger(state);
                state = PChanger(state);
                state = LChanger(state);
                key = KeySchedule(key, i);
                state = XChanger(state, key);
            }
            return state;
        }
        ...
        {
            var key = XChanger(h, N);
            key = SChanger(key);
            key = PChanger(key);
            key = LChanger(key);
            var t = Encryption(key, m);
            t = XChanger(t, h);
            var newh = XChanger(t, m);
            return newh;
        }
        public static byte[] GetHash(byte[] message, int outLen)
        {
            ...
            else if (outLen == 256)
            {
                for (int i = 0; i < 64; i++)
                {
                    N[i] = 0x00;
                    Sigma[i] = 0x00;
                    initialVector[i] = 0x01;
                }
            }

            byte[] h = new byte[64];
            Array.Copy(initialVector, h, 64);
            byte[] N_512 = BitConverter.GetBytes(512);
            N_512 = N_512.Reverse().ToArray();
            int inc = 0;
            while (len >= 512)
            {
                inc++;
